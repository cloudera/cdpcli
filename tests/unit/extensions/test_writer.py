...
class TestConfigFileWriter(unittest.TestCase):
    ...
    def test_new_config_file_in_new_dir(self):
        config_content = {'foo': 'bar'}
        new_config_name = os.path.join(self.config_filename, "foobar/foo.conf")
        self.writer.update_config(config_content, new_config_name)
        ...
        expectec_context = \
            '[default]\n' \
            'foo = bar\n'
        if new_contents != expectec_context:
            self.fail("Config file contents do not match.\n"
                      "Expected contents:\n"
                      "%s\n\n"
                      "Actual Contents:\n"
                      "%s\n" % (expectec_context, new_contents))
