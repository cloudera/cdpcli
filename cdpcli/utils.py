...
def parse_timestamp(value):
    ...
    if isinstance(value, (int, float)):
        return datetime.datetime.fromtimestamp(value, compat_tzlocal())
    else:
        try:
            return datetime.datetime.fromtimestamp(float(value), compat_tzlocal())
...
class ArgumentGenerator(object):
    ...
    ISO8601 is used. The intended usage of this class is to generate the *shape*
    ...
    def generate_skeleton(self, shape):
        ...
        elif shape.type_name == MAP_TYPE:
            return self._generate_type_map(shape)
