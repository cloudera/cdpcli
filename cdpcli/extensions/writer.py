...
class ConfigFileWriter(object):
    ...
    def update_config(self,
        ...
        * Any existing lines that are specified by ``new_values``
        ...
        :type config_file_comment: str
        :param config_file_comment: A config file comment that will written
            at the beginning of the ini file. This will only be written out
            if we are creating a new configuraion file.
        """
        section_name = new_values.pop('__section__', DEFAULT_PROFILE_NAME)
        if not os.path.isfile(config_filename):
            self._create_file(config_filename, config_file_comment)
    ...
    def delete_config_section(self, section_name, config_filename):
        """
        Helper function to delete a specified section from the config file
        :param section_name: Name of the section.
        :param config_filename: Config file location.
        """
        if not os.path.isfile(config_filename):
            return
        with open(config_filename, 'r') as f:
            contents = f.readlines()
        try:
            self._delete_section_contents(contents, section_name)
            with open(config_filename, 'w') as f:
                f.write(''.join(contents))
        except SectionNotFoundError:
            # Section doesn't exist so simply pass
            pass

    def _create_file(self, config_filename, config_file_comment=None):
        ...
        with os.fdopen(os.open(config_filename,
            ...
            if config_file_comment is not None:
                for line in config_file_comment.split('\n'):
                    f.write('# %s\n' % line)
    ...
    def _delete_section_contents(self, contents, section_name):
        # ``contents`` is a list of file line contents.
        section_start_line_num = self._find_section_start(contents,
                                                          section_name)
        section_end_line_num = None
        current_line_num = section_start_line_num + 1
        while current_line_num < len(contents):
            line = contents[current_line_num]
            if self.SECTION_REGEX.search(line) is not None:
                # We've hit a new section which means the earlier section is complete.
                section_end_line_num = current_line_num
                break
            current_line_num += 1
        # Remove the range from section start to end
        contents[section_start_line_num:section_end_line_num] = []
