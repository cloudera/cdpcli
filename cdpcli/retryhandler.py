...
def create_retry_handler(config, operation_name=None):
    checker = create_checker_from_retry_config(config,
                                               operation_name=operation_name)
    action = create_retry_action_from_config(config)
    return RetryHandler(checker=checker, action=action)


def create_retry_action_from_config(config):
    # Currently we assume the config comes from the default section, which means
    # that delay functions apply for every policy in the retry config (per
    # service).
...
def _create_single_response_checker(response):
    if 'service_error_code' in response:
        ...
        raise ValueError("Unknown retry policy: %s" % response)
    return checker


def _extract_retryable_exception(config):
    applies_when = config['applies_when']
    if 'socket_errors' in applies_when:
        exceptions = []
...
class RetryHandler(object):
    ...
    def __call__(self, attempts, response, caught_exception):
        if self._checker(attempts, response, caught_exception):
            result = self._action(attempts=attempts)
            LOG.info("Retry needed, action of: %s", result)
            return result
...
class BaseChecker(object):
    ...
    def __call__(self, attempt_number, response, caught_exception):
        ...
        attempt_number: The total number of times we've attempted to send the
                        request.
        response: The HTTP response (if one was received).
        caught_exception: Any exception that was caught while trying to send the
                          HTTP response.
    ...
    def __call__(self, attempt_number, response, caught_exception):
        ...
        if should_retry:
            if attempt_number >= self._max_attempts:
                LOG.info("Reached the maximum number of retry attempts: %s",
                         attempt_number)
                return False
            else:
                return should_retry
    ...
    def _should_retry(self, attempt_number, response, caught_exception):
        if self._retryable_exceptions and \
            ...
            try:
                ...
                LOG.info("retry needed, retryable exception caught: %s", e,
                         exc_info=True)
                return True
        else:
...
class HTTPStatusCodeChecker(BaseChecker):
    ...
    def _check_response(self, attempt_number, response):
        if response[0].status_code == self._status_code:
            LOG.info("retry needed: retryable HTTP status code received: %s",
                     self._status_code)
            return True
        else:
            return False
